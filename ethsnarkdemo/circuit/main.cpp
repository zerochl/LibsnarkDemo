...
std::string getBaseFileName(unsigned int blockType, unsigned int blockSize) {
...
void ZeroDepositSetup(char **argv) {
    std::cout << "in ZeroDepositSetup" << std::endl;
    // blockType其实并未使用到
    unsigned int blockType = 0;
    unsigned int blockSize = 8;
    std::string baseFilename = getBaseFileName(blockType, blockSize);
...
//#endif
    ...
    std::cout << "in ZeroDepositSetup end" << std::endl;
}

void ZeroDepositProve(char **argv) {
    std::cout << "in ZeroDepositProve" << std::endl;
    ...
    // 读取block 信息
    json input = loadJSON(argv[3]);
    if (input == json()) {
        std::cout << "in ZeroDepositProve block json error." << std::endl;
        return;
    }
    ...
    std::string jProof = proveCircuit(context, circuit);
    if (jProof.length() == 0) {
        std::cout << "in ZeroDepositProve jProof.length is 0" << std::endl;
        return;
    }
    if (!writeProof(jProof, proofFilename)) {
        std::cout << "in ZeroDepositProve writeProof error" << std::endl;
        return;
    }
#endif
}
...
void ZeroDepositVerify(char **argv) {
    ...
    std::cout << "in ZeroDepositVerify verify pass." << std::endl;
}

void ZeroSignatureSetup(char **argv) {
}

void ZeroSignatureProve(char **argv) {
}

void ZeroSignatureVerify(char **argv) {
}


void ZeroTest(char **argv) {
    std::cout << "in ZeroTest" << std::endl;
    if (std::string(argv[2])  == "setup") {
        ZeroDepositSetup(argv);
    } else if (std::string(argv[2])  == "prove") {
        ZeroDepositProve(argv);
    } else if (std::string(argv[2])  == "verify") {
        ZeroDepositVerify(argv);
    } else if (std::string(argv[2])  == "signatureSetup") {
        ZeroSignatureSetup(argv);
    } else if (std::string(argv[2])  == "signatureProve") {
        ZeroSignatureProve(argv);
    } else if (std::string(argv[2])  == "signatureVerify") {
        ZeroSignatureVerify(argv);
    }
}
